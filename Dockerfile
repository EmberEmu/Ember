...
RUN --mount=type=cache,target=/var/lib/apt/lists \
    ...
    apt-get -y install libbotan-2-dev && \
    apt-get -y install libmysqlcppconn-dev && \
    apt-get -y install zlib1g-dev && \
    apt-get -y install libpcre3-dev && \
    apt-get -y install libflatbuffers-dev

# Download, extract, and build Boost using its own cache, and force a commit of its cache into a layer
RUN --mount=type=cache,target=/var/cache/boost \
    wget -q -O /var/cache/boost/boost.tar.gz https://archives.boost.io/release/1.87.0/source/boost_1_87_0.tar.gz && \
    mkdir -p /tmp/boost && \
    tar -xzf /var/cache/boost/boost.tar.gz -C /tmp/boost && \
    cd /tmp/boost/boost_1_87_0 && \
    ./bootstrap.sh --with-libraries=system,program_options,headers && \
    ./b2 install -d0 -j$(nproc) cxxflags="-std=c++23"
...
# Generate Makefile & compile with caching enabled for the build directory
RUN --mount=type=cache,target=${working_dir}/build \
    cmake -S . -B build \
