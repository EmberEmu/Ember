...
public:
	explicit BinaryOutStream(Buffer& source, std::size_t read_limit = 0)
                             : buffer_(source), total_read_(0),
                               read_limit_(read_limit), state_(State::OK) {}

	// terminates when it hits a null-byte or consumes all data in the buffer
	BinaryOutStream& operator >>(std::string& dest) {
		char byte;
	...
	BinaryOutStream& operator >>(trivially_copyable auto& data) {
