...
#define PACKET_TRACE(ctx, fmt_str, ...) \
	LOG_FTRACE(ctx.logger, log::Filter(lf_packet_trace), "{}: " fmt_str, ctx.whoami(), ##__VA_ARGS__)

#define CLIENT_TRACE(ctx, fmt_str, ...) \
	LOG_TRACE(ctx.logger, "{}: " fmt_str, ctx.whoami(), ##__VA_ARGS__)

#define CLIENT_DEBUG(ctx, fmt_str, ...) \
	LOG_DEBUG(ctx.logger, "{}: " fmt_str, ctx.whoami(), ##__VA_ARGS__)

#define CLIENT_INFO(ctx, fmt_str, ...) \
	LOG_INFO(ctx.logger, "{}: " fmt_str, ctx.whoami(), ##__VA_ARGS__)

#define CLIENT_WARN(ctx, fmt_str, ...) \
    LOG_WARN(ctx.logger, "{}: " fmt_str, ctx.whoami(), ##__VA_ARGS__)

#define CLIENT_ERROR(ctx, fmt_str, ...) \
	LOG_ERROR(ctx.logger, "{}: " fmt_str, ctx.whoami(), ##__VA_ARGS__)

#define CLIENT_FATAL(ctx, fmt_str, ...) \
	LOG_FATAL(ctx.logger, "{}: " fmt_str, ctx.whoami(), ##__VA_ARGS__)
