...
int main(int argc, const char* argv[]) try {
	print_banner(realm::app_name);
	utility::set_window_title(realm::app_name);
...
int run(const opts::variables_map& args, log::Logger& logger, commands::Command& registry) try {
	...
	realm::Service service(logger, registry);
...
opts::variables_map parse_arguments(const int argc, const char* argv[]) {
	...
	cmdline_opts.add_options()
		...
		("config,c", opts::value<std::string>()->default_value("realm.conf"),
	...
	//Config file options
	opts::options_description opts("Realm configuration options");
	opts.add(realm::Service::options());
	opts.add_options()
