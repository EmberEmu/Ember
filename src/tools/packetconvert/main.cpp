...
void validate(boost::any& v, const std::vector<std::string>& values, OutputOption*, int) {
	...
	const auto& option = opts::validators::get_single_string(values);

	if(option == "console") {
		v = boost::any(option);
	} else {
		throw opts::validation_error(opts::validation_error::invalid_option_value);
	}
}
