...
TEST(srp6a, RFC5054_TestVectors) {
	...
	EXPECT_EQ(expected_key, Botan::BigInt::from_bytes(std::span<const std::uint8_t>(c_sess_key.data(), c_sess_key.size())))
		<< "Client key did not match expected value!";
	EXPECT_EQ(expected_key, Botan::BigInt::from_bytes(std::span<const std::uint8_t>(s_sess_key.data(), s_sess_key.size())))
...
TEST(srp6Regressions, NPad_GenerateClientProof) {
	...
	session_val.serialize_to({ kt.data() + (kt.size() - session_val.bytes()), session_val.bytes() });
